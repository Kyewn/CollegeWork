  ...
  while (!rates[RATE_SIZE-1] > 0) {
    ...
    if (isFingerDetected) {
      if (checkForBeat(irValue) == true)
      {
        ...
        {
          if (beatsPerMinute-60 < 60) {
            // Low BPM
            lowBPMChannel.publish(beatsPerMinute-60);
          } else if (beatsPerMinute-60 > 100) {
            // High BPM
            highBPMChannel.publish(beatsPerMinute-60);
          } else {
            // Normal BPM
            normalBPMChannel.publish(beatsPerMinute-60);
          }
