...
function countryFlags2(element) {
  for (let code in country_currencies) {
    if (code == element.value) {
      ...
      imgTag.src = `https://www.countryflags.io/${country_currencies[code]}/flat/48.png`;
    }
  }
}
...
function getListings() {
  ...
  let url = `https://v6.exchangerate-api.com/v6/a3bb9fc63fdae6c52bb81c7a/latest/${fromCurrency2.value}`;
  fetch(url)
  ...
  .then((result) => {
    ...
    for (let i=1; i<competitors.length; i++) {
        competitorList += `<tr><td><img src="./competitor_logos/${competitors[i][0]}" alt="Company logo"/></td>
        <td>${competitors[i][1]}</td>
        <td>${competitors[i][2]} ${competitors[i][2] == "-"? "" : fromCurrency2.value}</td></tr>`;
    } 
    let row =
      `<table class="rates-table">
        <tr class="column-header">
          <td>Provider</td>
          <td>Exchange Rate (<span style="color: rgb(193, 193, 255);">${fromCurrency2.value}</span>&#8594;<span style="color: #fd7f7f;">${toCurrency2.value}</span>)</td>
          <td>Transfer fee</td>
        </tr>
        <tr>
          <td><img src="./images/companylogo.png" alt="Company logo"/></td>
          <td>${exchangeRate}</td>
          <td>${competitors[0]} ${competitors[0] == "-"? "" : fromCurrency2.value}</td>
        </tr>`
        + competitorList +
        "</table>";
  ...
  .catch(() => {
    ...
    if (bottomSection.innerHTML)
      bottomSection.innerHTML = "";
    let row =
    `<div class="rates-nodata">
